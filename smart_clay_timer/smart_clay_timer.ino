...
void readTemp() {
  ...
  for (auto T: histogram){
    if (T.second == mode_count) {
      ...
      int T_constrain = constrain(T.first, round((temp-4)*2), round(temp*2) );
      perc_dry = (T_constrain - round((temp - 4) * 2) )/ 8 * 100;
      Serial.print("Percentage dry: ");
      Serial.println(perc_dry);      
      int i = map(T_constrain, round((temp-4)*2), round(temp*2), 1, 8);
